...
class GlobalState(object):
    ...
    def dump_catapult_trace(self, path, task_info, breakdowns=False, edges=None):
      ...
      try:
        ...
        while task_id not in nodes:
          def add_objects(task_id):
            ...
            for obj in objs:
                objects[obj] = task_profiles(task_id["get_arguments_start"])
                edges.append((task_id, obj))
          add_objects(task_id)
          queue.append(task_id)
          parent_id = task_info[task_id]["TaskSpec"]["ParentTaskID"]
          if parent_id in task_info:
            edges.append((task_id, parent_id))
            if recurse:
              for task, data in task_info.items():
